...
public class MultiLaneOffensive implements AbstractPlayer
{
  ...
  public MultiLaneOffensive (Integer pid)
  {
    id = pid;
  ...
  private double evaluate (int corridor)
  {
	...
	int[] closeToBaseAllies = new int[Utilities.NUMBER_OF_CORRIDORS];
	...
	{	
		...
		for (int i = 0; i < Utilities.NUMBER_OF_CORRIDORS; i++)
		{
			...
			else if (uni.getOwner() == id && uni.isMarine() == true && uni.getCorridor() == i) 				
			{
				...
				if (Utilities.getDistance(uni.getX(),uni.getY(),startX,startY) < 500) 
				{
	...
	if (allies[corridor] > 0 && laneEnemies[corridor] > 0)
	{
		...
		else if (allies[corridor] == laneEnemies[corridor])
		{
			...
			for (int i = 0 ; i < Utilities.NUMBER_OF_CORRIDORS ; i++)
			{
				...
				if (i != corridor)
				{
					...
					evaluation += 1 - 1*(allies[i]-laneEnemies[i]); // Otan exw pleonekthma thelw na stelnw sto pleonekthma kai oxi sthn isopalia
		...
		else
		{
			...
			evaluation = 0.1*(laneEnemies[corridor]-allies[corridor]);
	...
	else if (allies[corridor] > 0 && laneEnemies[corridor] == 0)
	{
		...
		for (int i = 0 ; i < Utilities.NUMBER_OF_CORRIDORS ; i++)
		{
			...
			if ((i != corridor) && (allies[i] - laneEnemies[i]) > 2 && flag == false) //Mas noiazei an mono 1 lane exei pleonekthma. 2 lane me pleonekthma den ta peirazoume (flag explanation)
			{

				evaluation += 1.1*(allies[i]-laneEnemies[i]);	//to 1.1 einai megalytero tou 1 opote h grammh me to pleonekthma tha parei xamhlotero evaluation
				flag = true;
	...
	else if (allies[corridor] == 0 && laneEnemies[corridor] == 0)
	{
		...
		for (int i = 0 ; i < Utilities.NUMBER_OF_CORRIDORS ; i++)
		{
			...
			if ((i != corridor) && (allies[i] - laneEnemies[i]) > 2 && flag == false) 
			{
				
				evaluation += 1.1*(allies[i]-laneEnemies[i]);
				flag = true;
