...
public class SimpleDefensive implements AbstractPlayer
{
  ...
  public SimpleDefensive (Integer pid)
  {
    id = pid;
  ...
  private double evaluate (int corridor)
  {
	...
	for (Unit uni : units) 
	{
		if (uni.getCorridor() == corridor) 	
		{
			if (uni.getOwner() != id && uni.isMarine() == true)
			{
			...
			else if (uni.getOwner() == id && uni.isMarine() == true) 
			{
