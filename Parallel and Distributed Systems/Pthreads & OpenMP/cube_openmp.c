...
int main() {
	...
	for (i=0;i<8;i++) {
		...
		arg[i].parent_points = NULL;				// The entire matrix A will be examined	
		arg[i].n_points = N;
	...
	int *sorted_ids;			// Array that holds the boxids sorted
	...
	int sorted_ids_pointer;						// sorted_ids index
	int local_pointer;							// Local copy of sorted_ids_pointer for each thread

	sorted_ids_pointer = 10;					// Set sorted_ids_pointer to 10 since the first 9 boxes are already set
	for(m=2;m<=max_level;m++) {					// For each level

	#pragma omp parallel for private(i,j,k,l,local_pointer) shared(box,sorted_ids,sorted_ids_pointer) schedule(static)

		for(i=0;i<ids_in_level[m];i++) {		// For each box in this level
			
			l=0;								// Reset colleague counter
			local_pointer = sorted_ids_pointer;	// Store a local copy of sorted_ids_pointer in this thread
			sorted_ids_pointer++;				// Incremebt sorted_ids_pointer by 1
